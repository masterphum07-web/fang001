...
export const DayDetailCard: React.FC<DayDetailCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-3xl sm:rounded-[2rem] p-5 sm:p-6 border border-slate-200 shadow-sm flex flex-col justify-between space-y-4">
      <div>
        ...
        <div className="flex items-center justify-between pb-3.5 border-b border-slate-100 mb-3.5">
          <div className="flex items-center gap-2 text-slate-900">
            ...
            <span className="font-extrabold text-base sm:text-lg">{formattedDateThai}</span>
          </div>

          <button
            onClick={() => onOpenLogModal(dateStr)}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-xl bg-slate-900 text-white hover:bg-slate-800 transition-colors shadow-2xs"
          >
            <Edit3 className="w-3.5 h-3.5" />
            <span>{log ? 'แก้ไข' : '+ บันทึก'}</span>
          </button>
        </div>
        ...
        <div className="space-y-2.5 text-xs sm:text-sm">
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/70">
            <div className="flex items-center gap-1.5 font-bold text-slate-800 mb-1">
              ...
              <span>ระดับฮอร์โมน & สรีรวิทยา:</span>
            </div>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/70">
            <div className="flex items-center gap-1.5 font-bold text-slate-800 mb-1">
              ...
              <span>ความรู้สึก & สภาพร่างกาย:</span>
            </div>
            <p className="text-slate-600 font-medium leading-relaxed text-xs">
              {phaseInfo.bodyFeel}
            </p>
          ...
          <div className="mt-4 pt-3 border-t border-slate-100 space-y-2">
            ...
            {log.symptoms.length > 0 && (
              <div className="flex flex-wrap gap-1.5 pt-1">
                {log.symptoms.map((s) => (
                  <span key={s} className="px-2 py-0.5 text-[11px] rounded-lg bg-amber-50 text-amber-900 border border-amber-200 font-bold">
                    {SYMPTOM_LABELS[s] || s}
                  </span>
                ))}
              </div>
            ...
            {log.partnerNote && (
              <div className="text-xs text-sky-900 bg-sky-50 p-2.5 rounded-xl border border-sky-200 flex items-start gap-1.5 font-semibold">
      ...
      <div className="mt-4 p-4 rounded-2xl bg-gradient-to-br from-sky-50 to-blue-50 border border-sky-100">
        <div className="flex items-center gap-1.5 text-xs font-bold text-sky-900 mb-1">
          ...
          <span>{profile.mode === 'partner' ? 'ข้อแนะนำดูแลแฟนวันนี้:' : 'ดูแลตัวเองในวันนี้:'}</span>
        </div>
